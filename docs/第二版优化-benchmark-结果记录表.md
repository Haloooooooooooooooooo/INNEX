...
## R9-QA（2026-05-31，Phase 5C QA 7 题复测，代码版本=R11 同期）

- 版本：R11 同期代码（embedding 维度修复 + LLM 判型否决权 + R7 收尾 + 5B 决策层补齐均已在代码中）
- 构建：`apps/web` `npm run build` 通过。
- 复测口径：同一套 7 题，不换题不换样本。

### QA 侧逐题结果

| # | 问题 | 意图判定 | 证据分 | 检索表现 | 回答质量 | 结果 |
|---|---|---|---|---|---|---|
| Q1 | 总结 Prompt AB 测试五步法 | ✅ 总结归纳 | 63% | K=10, 4条精准命中 AB 测试笔记 | 结构完整、证据充分 | ✅ 通过 |
| Q2 | 提升对话留存 AB 测试指标怎么设 | ✅ 事实查询 | 64% | K=5, 2条精准命中 AB 测试笔记 | 核心/辅助指标准确 | ✅ 通过 |
| Q3 | 和前一个问题相比最关键的控制变量 | ✅ 复盘反思 | 53% | K=9, 续问识别正确 | 正确对比两轮差异 | ✅ 通过 |
| Q4 | PRD 模板应包含哪些模块 | ✅ 事实查询 | 70% | K=5, 2条命中 PRD 笔记 | 9模块完整列出 | ✅ 通过 |
| Q5 | 继续 PRD 话题，给最小模板 | ❌ 续问断链 | 58% | K=5, 误召回 Agent 设计笔记 | 回答”无法提供” | ❌ 失败 |
| Q6 | 意图识别准确率评估看哪些指标 | ✅ 事实查询 | 71% | K=5, 2条命中意图识别笔记 | 指标+原因完整 | ✅ 通过 |
| Q7 | 意图识别和 PRD 有什么关联 | ✅ 复盘反思 | 54% | K=9, 召回7条不相关片段 | 正确判定”暂无直接关联” | ✅ 通过 |

**汇总：6/7 通过，1/7 失败（Q5）。**

### R7 三个 Badcase 复测结论

| 原 Badcase | 现象 | 复测结果 | 状态 |
|---|---|---|---|
| R7-C01 | Q2 召回偏题 | Q2 精准命中 AB 测试笔记，无偏题 | ✅ 已关闭 |
| R7-C02 | Q3/Q5 follow-up 断链 | Q3 续问正常；**Q5 仍断链** | ⚠️ 部分修复 |
| R7-C03 | Q7 拒答过硬 | Q7 正确输出”暂无直接关联” | ✅ 已关闭 |

### R9-QA Badcase

| Case ID | 问题 | 问题类型 | 现象描述 | 证据 | 原因标签 | 修复动作 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| R9-C01 | Q5（PRD最小模板） | 续问断链 | Q4 命中 PRD 笔记，Q5”继续上一个PRD话题”却召回到 Agent 设计笔记，回答”无法提供” | `qa检索测试.txt` Q5 | `qa_followup_misjudged` | 续问场景 Stage 3.7 复用结果排序加权（主检索可能覆盖了复用结果） | 已确认待修复 |

### 根因分析（Q5）

1. `FOLLOWUP_REFERENCE_MARKERS` 匹配”继续/上一个”，续问复用逻辑已触发。
2. 但 Stage 3.7 复用是对上轮引用 note 做 `match_note_chunks_in_notes` 补召回，结果需要与主检索结果 fuse 排序。
3. Q5 中”最小模板”被 bge-m3 匹配到 Agent 设计笔记（也讲方法论/模板），相似度可能高于 PRD 笔记中对应 chunk。
4. **核心矛盾**：续问复用正确注入了 PRD note 的 chunks，但在最终 fuse 排序中被主检索的错误高分结果压下去了。

**修复方向**：续问场景下对 Stage 3.7 复用结果给予排序加权（类似 `reorderChunksBySessionTopic` 的逻辑），而非仅平权融合。

### 检索层后续优化点

| 优化项 | 说明 | 优先级 |
|---|---|---|
| 续问复用结果排序加权 | Stage 3.7 复用的 chunks 在 fuse 排序时加成，确保不被主检索错误结果淹没 | 高（Phase 6 前修） |
| 低证据分提前截断 | Q7 证据分 54% 且 7 条全不相关，白白浪费 token | 中 |

### 回答生成层问题（为 Phase 6 准备）

从 7 题回答中识别出 6 个系统性问题，详见全模块迭代计划 §10.6：

1. 回答结构机械模板化（简单问题也输出完整四段）
2. 暴露检索调试信息（意图/证据分/K值/阈值）
3. 引用格式像论文（`[1] 第3-7步`而非自然对话）
4. “可执行下一步”机械复述（Q1/Q2/Q3 几乎相同）
5. “不确定项”过度展开（每题凑3条）
6. 续问断链时拒答体验差（Q5 直接说”无法提供”）

### R9-QA 关闭/新增确认
- R7-C01（Q2 召回偏题）：✅ **已关闭** — 同会话主题词重排生效。
- R7-C02（Q3/Q5 follow-up 断链）：⚠️ **部分关闭** — Q3 修复，Q5 仍断链，需续问复用结果加权。
- R7-C03（Q7 拒答过硬）：✅ **已关闭** — 无关联判定兜底生效。
...
## R12-QA（2026-06-01，Phase 6 第三轮 QA 7 题复测）
...
逐题结论：
1. Prompt AB 五步法总结：通过
2. 留存实验指标设计：通过
3. 控制变量追问：通过
4. PRD 模块问题：通过
5. 续问 PRD 最小模板：通过（已延续上文，不再跑偏）
6. 意图识别评估指标：通过
7. 与 PRD 是否关联：通过（明确“没有关联”）
